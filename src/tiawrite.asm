...
;*
;* automatic adjustment of game colors
;*
...
; 	mov	dword [TempCFirst],edx		; to see how much we're changing by
	...
	jne near VSyncAlreadySet		; don't change video mode if not matching previous frame (quadrun)

 	test	byte [IOPortB],1		; reset being pressed?
	jz near VSyncAlreadySet			;   yes, don't adjust

AdjustUnstable:
 	cmp	dword [LinesInFrame],287	; 282 NTSC game? (pharhcrs 296 when fire button pressed ...
						; (... air_raid 292, zoofun 291, dumbo 286, curtiss 286,
						; (... tps 285, galaga 282, tomboy 277, bermuda 286)
	jb near ShouldBeNTSC			;   yes

 	cmp	byte [PaletteNumber],1		; PAL mode already ?
	jz near VSyncAlreadySet			;   yes
 	cmp	byte [UserPaletteNumber],0ffh	; is there a palette override?
	jnz near VSyncAlreadySet	 	;   yes, don't switch
 	cmp	byte [GamePaletteNumber],0ffh
	jnz near VSyncAlreadySet		;   yes, don't switch
	

 	test	dword [Frame],0ffffff00h 	; 0ffffff00h more than 256 frames passed?
	jnz near VSyncAlreadySet	 	;   yes, don't change TV type *EST*

 	mov	byte [PaletteNumber],1		; set up PAL palette
	pushad
	call	CreateScreen
	call	srv_SetPalette	 		;  reset palette for PAL games
	call	ClearScreenBuffers
;	call	srv_SetVideoMode
	call	position_game
	popad
	jmp	VSyncAlreadySet

ShouldBeNTSC:
	cmp	byte [PaletteNumber], 0
	jz near VSyncAlreadySet	
 	mov	byte [PaletteNumber],0		; set up NTSC palette
	pushad
	call	CreateScreen
	call	srv_SetPalette	 		;  reset palette for PAL games
	call	ClearScreenBuffers
...
UBGnoball:
	DeactivateObject  BL_BIT		; no ball, DeactivateObject
 	mov	byte [BL_Graphics],0		; clear the graphics register
...
%macro UpdateMissileGraphics 1
	...
	DeactivateObject  %1_BIT		; assume inactive
 	mov	byte [%1_Graphics],0		; clear register
...
SetupMultiSpriteTrick:
 	cmp	byte [M0_TripleFlag],1
	je near SMS_M1
 	mov	byte [M0_TripleFlag],1
	call	UpdateM0Table

SMS_M1: cmp	byte [M1_TripleFlag],1
	je near SMS_P0
 	mov	byte [M1_TripleFlag],1
	call	UpdateM1Table

SMS_P0: cmp	dword [P0_TripleFlag],1
	je near SMS_P1
 	mov	byte [P0_TripleFlag],1
	call	UpdateP0Table

SMS_P1: cmp	dword [P1_TripleFlag],1
	je near SMS_done
 	mov	byte [P1_TripleFlag],1
	call	UpdateP1Table
...
%macro DoMotion 1
 	...
 	cmp	dword [%1_Position],68
	jae near %%positive
 	add	dword [%1_Position],160
	jmp	%%done

%%positive:
 	cmp	dword [%1_Position],228
	jb near %%done
 	sub	dword [%1_Position],160
%%done:
%endmacro
...
%%NoCatchup:
	...
	push	edx				; save current player graphics
 	mov	byte [P%1_Graphics],0		; set to zero to render nothing
	GetObjectDelay  P%1
